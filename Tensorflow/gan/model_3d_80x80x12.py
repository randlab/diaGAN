...
model_config = {
    "data_dimension" : (80,80,12),
    "generator_output_dimension" : (80,80,12),
    "noise_dimension" : (256,),
    "output_in_3D" : True,
    "scale_free" : False
}
...
def make_generator(noise_dim=model_config["noise_dimension"]):
    ...
    model.add(Dense(300, input_dim=noise_dim[0]))
    model.add(LeakyReLU(0.2))

    model.add(Reshape((5,5,3,4), input_shape=(300,)))

    model.add( Conv3DT(64, (5, 5, 5), strides=2, padding='same', kernel_initializer='he_normal'))
    model.add( BatchNormalization())
    model.add( LeakyReLU(alpha=0.2))

    model.add( Conv3D(128, (5, 5, 5), padding='same', kernel_initializer='he_normal'))
    model.add( BatchNormalization())
    model.add( LeakyReLU(alpha=0.2))

    model.add( Conv3DT(256, (4, 4, 4), strides=2, padding='same', kernel_initializer='he_normal'))
    model.add( BatchNormalization())
    model.add( LeakyReLU(alpha=0.2))

    model.add( Conv3DT(512, (3, 3, 3), strides=[2,2,1], padding='same', kernel_initializer='he_normal'))
    model.add( BatchNormalization())
    model.add( LeakyReLU(alpha=0.2))
    ...
    model.add( Conv3DT(128, (5, 5, 5), strides=[2,2,1], padding='same', kernel_initializer='he_normal'))
    model.add( BatchNormalization())
    model.add( LeakyReLU(alpha=0.2))
